...
3. **Scoring engine** — pure functions (`sleepScore`, `nutritionScore`, `exerciseScore`, `stressScore`, `totalScore`, `scoreColor`, `DEFAULT_GOALS`). Metric capture helpers (`clockToMinutes`, `hoursBetweenClock`, `sleepHoursFromClock`, `fastingHoursFromClock`, `nutritionQualityFromChecklist`, option tables) live in the same section; `dailyInsight` lives in Streaks.
4. **Streaks & weekly comparison** — `computeStreak`, `weeklyComparison`, `dailyInsight` (the daily insight card's variable-message picker).
5. **Recommendation engine** — `tier`, `RECS`, `buildRecommendations`, `PROTOCOLS`/`ProtocolCard`.
6. **Body composition → nutrition direction** — `BODY_STAGES` (6 stages, two axes: 1-4 body fat, 5-6 muscle) + `BODY_STAGE_AXES`; `goalDirectionFromBody` returns `cut | bulk | recomp | maintain`, backed by `profiles.body_scale_version = 2`; `startingPointDiagnosis` gives BMR/TDEE/estimated fat %/estimated metabolic age from the profile alone; `CATEGORY_TO_SUPPLEMENT_GOAL`, `adaptiveNoteApplies`.
7. **UI components** — `SimpleLineChart`, `ScoreRing`, `FieldSlider`, `ClockPairField`, `ChoiceField`, `MinutesField`, `ChecklistField`, `BodySelector`/`BodyImageButton`, `AuthScreen`, `Dashboard`, `App`.
...
- **`profiles` table**: one row per user — per-user goals (`goal_sleep_hours`, `goal_fasting_hours`, `goal_exercise_minutes`, `goal_stress_max`), body-composition selection (`body_gender`, `body_current_stage`, `body_target_stage`, `body_scale_version`), the daily reminder preference (`reminder_time`, nullable `time`), and premium bookkeeping (`plan` is a legacy/display-only column — **do not gate on it**; `premium_source` is `'paid' | 'comp_trainer' | 'trial' | null` and `trial_ends_at` backs the `trial` case). Read via `GET {REST}/profiles?id=eq.{userId}&select=...`, written via `PATCH {REST}/profiles?id=eq.{userId}`.
- **`daily_entries` table**: one row per user per day (`entry_date`), holding raw metric inputs (`sleep_hours`, `sleep_quality`, `fasting_hours`, `nutrition_quality`, `exercise_minutes`, `exercise_intensity`, `stress_level`) plus the raw clock/checklist answers they're derived from (`sleep_bedtime`, `sleep_waketime`, `fast_last_meal`, `fast_first_meal`, `nutrition_checks`) — the derived hour/quality values are computed client-side (metric capture helpers, Scoring engine) before the upsert. Upserted via `POST {REST}/daily_entries?on_conflict=user_id,entry_date` with `Prefer: resolution=merge-duplicates`. Read via `GET {REST}/daily_entries?user_id=eq.{userId}&order=entry_date.asc&limit=60`.
...
- **`coaches`** (id = `profiles.id`; roster + workout-routine business, flat $20/mo per coach — `subscription_tier` check constraint is limited to `'standard'`) — read own row via `GET {REST}/coaches?id=eq.{userId}`. **No INSERT policy exists** on the table itself; coach status is granted via the `public.admin_set_coach_status(p_user_id, p_active, p_business_name)` `SECURITY DEFINER` RPC (gated by `public.is_admin()`), wired to the "Hacer coach" / "Quitar como coach" toggle in `AdminCoachControls` — there is no self-serve signup for the *coach themselves*, but Mario/admins no longer need raw SQL to grant it. Two Postgres triggers enforce hard caps: `trg_enforce_coach_client_limit` (max 25 clients per coach, on `coach_clients`) and `trg_enforce_active_coach_cap` (max 50 active coaches platform-wide, on `coaches`).
...
Premium status is determined by the Postgres function `public.is_premium(profile_id uuid)` (`SECURITY DEFINER`, checks `premium_source`/`trial_ends_at`), called via `POST {REST}/rpc/is_premium` with body `{ profile_id: session.user.id }` inside `loadPlan()`. The result is stored in `Dashboard`'s `isPremium` state and is the **only** thing gating section rendering — never gate new premium content on `profiles.plan`. Free: score, daily entry, goals, daily recommendations, daily insight, reminder. Premium (`isPremium`): diet plans, meal scanner, recipes (pro+), supplements, 7-day protocols. Free users see a blurred teaser + "Desbloquear con Premium" CTA in the same visual pattern per section; follow that pattern for any new premium section rather than hiding it outright.
